...
describe('POST /create', () => {
  it('should create a new user and return 201', async () => {
    ...
    (generatePasswordHash as jest.Mock).mockImplementation(
    ...
    expect(createUserRequestSchema.parse).toBeCalledWith({
      name: 'John Doe',
      password: 'Password123',
      email: 'johnDoe@gmail.com',
    });
    expect(generatePasswordHash).toBeCalledWith('Password123');
    expect(createNewUser).toBeCalledWith({
      name: 'John Doe',
      password: 'Password123',
      email: 'johnDoe@gmail.com',
    });
...
describe('POST /login', () => {
  it('Should successfully login and return a jwt token', async () => {
    ...
    expect(JsonApiResponse).toHaveBeenCalledWith(
      expect.anything(),
      'Login Successful',
      true,
      { token: mockJtwToken },
